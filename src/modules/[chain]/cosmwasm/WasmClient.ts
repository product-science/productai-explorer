...
export class WasmRestClient extends BaseRestClient<WasmRequestRegistry> {
  ...
  getWasmContractSmartQuery(address: string, query: string) {
    try {
      ...
      console.warn('Failed to execute smart query:', error);
      throw error;
    }
  }
  async getWasmContractQueries(address: string) {
    const dummy = JSON.stringify({ invalid_variant: {} });
    const query_data = toBase64(new TextEncoder().encode(dummy));
    ...
    try {
      ...
      const re = /`(\w+)`/g;
      const matches = String(error).match(re);
      return matches ? matches.map(e => e.replace(/`/g, '')) : [];
    }
  }
